    ...
    public class ArticleManager : IArticleService
    {
        ...
        public async Task<IResult> Delete(int ArticleId, string modifiedByName)
        {
            ...
            if (result)
            {
                ...
                await _unitOfWork.Articles.UpdateAsync(article).ContinueWith(a => _unitOfWork.SaveAsync());
        ...
        {
            var articles = await _unitOfWork.Articles.GetAllAsync(null, ar => ar.User, ar => ar.Category);
            if (articles.Count > -1)
            {
        ...
        public async Task<IDataResult<ArticleListDto>> GetAllByCategory(int categoryId)
        {
            ...
            if (result)
            {
                var articles = await _unitOfWork.Articles.GetAllAsync(a => a.CategoryId == categoryId && !a.IsDeleted && a.IsActive, ar => ar.Category, ar => ar.User);
                if (articles.Count > -1)
                {
                    return new DataResult<ArticleListDto>(ResultStatus.Success, new ArticleListDto
                    {
        ...
        {
            var articles = await _unitOfWork.Articles.GetAllAsync(a => !a.IsDeleted && a.IsActive, ar => ar.Category, ar => ar.User);  // a.IsActive == true dememize gerek yok, defaultta true
            if (articles.Count > -1)
            {
                return new DataResult<ArticleListDto>(ResultStatus.Success, new ArticleListDto
                {
        ...
        public async Task<IResult> HardDelete(int ArticleId)
        {
            ...
            {
                var article = await _unitOfWork.Articles.GetAsync(a => a.Id == ArticleId);
                await _unitOfWork.Articles.DeleteAsync(article).ContinueWith(a => _unitOfWork.SaveAsync());
